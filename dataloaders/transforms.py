...
class RandomRotate:
    ...
    def __call__(self, rgb,nir,red, mask):
        angle = random.randint(self.min_angle, self.max_angle)
        rgb  = F.rotate(rgb,angle)
        nir  = F.rotate(nir,angle)
        red  = F.rotate(red,angle)
        mask = F.rotate(mask,angle)
        return(rgb,nir,red,mask)
...
class CenterCrop:
    ...
    def __call__(self, rgb,nir,red, mask):
        ...
        nir = F.center_crop(nir, self.size)
        red = F.center_crop(red, self.size)
